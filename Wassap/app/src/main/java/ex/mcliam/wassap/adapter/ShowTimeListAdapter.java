...
import ex.mcliam.wassap.model.Showtime;
import ex.mcliam.wassap.util.FlowLayout;
...
public class ShowTimeListAdapter extends RecyclerView.Adapter<ShowTimeListAdapter.ShowtimeViewHolder>{
    ...
    public ShowtimeViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.activity_list_items_showtime, parent, false);
    ...
    public void onBindViewHolder(ShowtimeViewHolder holder, int position) {
        ...
        for (int i=0; i<showtime.getJam().size(); i++) {
            View view = LayoutInflater.from(context).inflate(R.layout.activity_list_items_time, holder.lyTime, false);
